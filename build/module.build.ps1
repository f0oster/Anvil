...
# Settings (user-editable in build.settings.psd1, defaults in build.settings_DEFAULTS_DO_NOT_EDIT.psd1)
$script:DefaultSettings = Import-PowerShellDataFile -Path (Join-Path -Path $PSScriptRoot -ChildPath 'build.settings_DEFAULTS_DO_NOT_EDIT.psd1')
$script:UserSettings    = Import-PowerShellDataFile -Path (Join-Path -Path $PSScriptRoot -ChildPath 'build.settings.psd1')

$script:ValidValues = @{
    TestOutputFormat = @('NUnitXml', 'NUnit2.5', 'NUnit3', 'JUnitXml')
    TestVerbosity    = @('None', 'Normal', 'Detailed', 'Diagnostic')
    LintFailOn       = @('Error', 'Warning', 'Information', 'ParseError')
}

function Test-SettingValid ([string]$Name, $Value) {
    if ($null -eq $Value) { return $false }
    switch ($Name) {
        'ModuleName'        { return ($Value -is [string]) -and ($Value.Length -gt 0) }
        'CoverageThreshold' { return ($Value -is [int]) -and ($Value -ge 0) -and ($Value -le 100) }
        'IncludeDocs'       { return $Value -is [bool] }
        'TestOutputFormat'  { return $Value -in $script:ValidValues.TestOutputFormat }
        'TestVerbosity'     { return $Value -in $script:ValidValues.TestVerbosity }
        'LintFailOn'        { return ($Value -is [array]) -and @($Value | Where-Object { $_ -notin $script:ValidValues.LintFailOn }).Count -eq 0 }
        'AssetDirectories'  { return ($Value -is [array]) -and @($Value | Where-Object { -not ($_ -is [string]) -or $_.Length -eq 0 }).Count -eq 0 }
        default             { return $true }
    }
}
...
foreach ($Key in $script:DefaultSettings.Keys) {
    if ($script:UserSettings.ContainsKey($Key) -and (Test-SettingValid -Name $Key -Value $script:UserSettings[$Key])) {
        $script:Settings[$Key] = $script:UserSettings[$Key]
    } else {
        if ($script:UserSettings.ContainsKey($Key)) {
            Write-Build Yellow "  [settings] Invalid value for '$Key' in build.settings.psd1 -- using default"
        }
...
$script:IncludeDocs       = $script:Settings.IncludeDocs
$script:TestOutputFormat  = $script:Settings.TestOutputFormat
$script:TestVerbosity     = $script:Settings.TestVerbosity
$script:LintFailOn        = $script:Settings.LintFailOn
$script:AssetDirectories  = $script:Settings.AssetDirectories
...
task Test {
    ...
    $Cfg.Run.Exit                           = $true
    $Cfg.Output.Verbosity                   = $script:TestVerbosity
    $Cfg.TestResult.Enabled                 = $true
    $Cfg.TestResult.OutputFormat             = if ($env:PESTER_OUTPUT_FORMAT) { $env:PESTER_OUTPUT_FORMAT } else { $script:TestOutputFormat }
...
task Docs {
    ...
    if (-not $script:IncludeDocs) {
        Write-Build DarkGray '  Docs disabled in build.settings.psd1 -- skipping'
        return
    }
...
task Build {
    ...
    # Copy static assets
    foreach ($AssetDir in $script:AssetDirectories) {
...
task Publish {
    ...
    Publish-PSResource -Path $Staging -ApiKey $ApiKey -Repository PSGallery -ErrorAction Stop
