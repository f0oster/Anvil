...
task Build {
    ...
    # Copy static assets
    foreach ($AssetDir in @('en-US', 'Types', 'Formats', 'Assemblies', 'Templates')) {
    ...
    if ($PSData.Prerelease)                    { $ManifestParams.Prerelease = $PSData.Prerelease }
...
task Version {
    ...
    if ($PSBoundParameters.ContainsKey('Prerelease')) {
        Update-ModuleManifest -Path $script:ManifestPath -Prerelease $Prerelease
        if ($Prerelease) {
            Write-Build Green "  Updated prerelease: $Prerelease"
        } else {
            Write-Build Green '  Cleared prerelease label'
        }
    }
    if (-not $NewVersion -and -not $PSBoundParameters.ContainsKey('Prerelease')) {
        Write-Build DarkGray '  Pass -NewVersion and/or -Prerelease to update'
    }
